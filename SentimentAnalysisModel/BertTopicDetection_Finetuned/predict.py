...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prediction using local/cached/remotely loaded Chinese BERT classification model")
    parser.add_argument("--model_root", type=str, default="./model", help="Local model root directory")
    parser.add_argument("--finetuned_subdir", type=str, default="bert-chinese-classifier", help="Fine-tuning results subdirectory")
    parser.add_argument("--pretrained_name", type=str, default="google-bert/bert-base-chinese", help="Pretrained model name or path")
    parser.add_argument("--text", type=str, default=None, help="Directly enter a text to be predicted")
...
def load_finetuned(model_root: str, subdir: str) -> Tuple[str, Dict[int, str]]:
    ...
    if not os.path.isdir(finetuned_path):
        raise FileNotFoundError(
            f"Fine-tuned model directory not found: {finetuned_path}, please run the training script first."
        )
...
def main() -> None:
    ...
    # Make sure the base model is local
    ...
    # Enter interactive mode by default (without explicitly specifying --text and not explicitly turning off interaction)
    if args.interactive or (args.text is None):
        print("Enter interactive mode. Type 'q' to exit.")
        while True:
            try:
            ...
            for rank, (label, conf) in enumerate(topk, 1):
                print(f"{rank}. {label} (p={conf:.4f})")
        return
    # Theoretically it won't get here
