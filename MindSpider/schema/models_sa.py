"""MindSpider database ORM model (SQLAlchemy 2.x)

This module defines the ORM model for the MindSpider extension table (decoupled from the original MediaCrawler table).
...
class DailyNews(Base):
    ...
    __table_args__ = (
        UniqueConstraint("news_id", name="uq_daily_news_id_unique"),  # Add unique constraints to foreign key references
...
class DailyTopic(Base):
    ...
    __table_args__ = (
        UniqueConstraint("topic_id", name="uq_daily_topics_id_unique"),  # Add unique constraints to foreign key references
